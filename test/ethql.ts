...
describe("ethql", function () {
  ...
  async function deployWithDependencies() {
    ...
    return await ETHQL.deploy()
  }

  it("creates table", async function() {
    const ethql = await loadFixture(deployWithDependencies);

    const txn = await ethql.getFunction("createTable").send("test", Buffer.from("0xff", "hex"));
